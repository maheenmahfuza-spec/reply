...
export async function generateProductPhotography(req: ImageRequest): Promise<string> {
  ...
  try {
    ...
    if (!response.candidates || response.candidates.length === 0) {
      throw new Error("No candidates returned from the model. This might be due to safety filters.");
    }
    ...
    throw new Error("The model responded but did not include an image part.");
  } catch (error: any) {
